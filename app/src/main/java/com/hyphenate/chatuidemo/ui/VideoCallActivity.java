...
public class VideoCallActivity extends CallActivity implements OnClickListener {
    ...
    class BrightnessDataProcess implements EMCameraDataProcessor {
        ...
        @Override public synchronized void onProcessData(byte[] data, Camera camera, final int width, final int height,
                final int rotateAngel) {
            int wh = width * height;
            for (int i = 0; i < wh; i++) {
    ...
    void addCallStateListener() {
        callStateListener = new EMCallStateChangeListener() {
            ...
            @Override public void onCallStateChanged(final CallState callState, final CallError error) {
                switch (callState) {
                    ...
                    case ACCEPTED: // call is accepted
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            @Override public void run() {
                                ...
                                openSpeakerOn();
                                ((TextView) findViewById(R.id.tv_is_p2p)).setText(
                                        EMClient.getInstance().callManager().isDirectCall() ? R.string.direct_call
                                                : R.string.relay_call);
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            monitorTextView.setText("WidthxHeight："
                                    + callHelper.getVideoWidth()
                                    + "x"
                                    + callHelper.getVideoHeight()
                                    + "\nDelay："
                                    + callHelper.getVideoLatency()
                                    + "\nFramerate："
                                    + callHelper.getVideoFrameRate()
                                    + "\nLost："
                                    + callHelper.getVideoLostRate()
                                    + "\nLocalBitrate："
