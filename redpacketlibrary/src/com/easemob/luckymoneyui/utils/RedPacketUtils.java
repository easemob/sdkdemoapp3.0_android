...
public class RedPacketUtils {
     ...
     * @param requestCode
     */
    public static void startMoneyActivityForResult(Fragment fragment, int chatType, String toChatUsername, int requestCode) {
        //发送者头像url
        ...
        Intent intent = new Intent(fragment.getContext(), LMMoneyActivity.class);
    ...
    public static EMMessage createRPMessage(Context context, Intent data, String toChatUsername) {
        String greetings = data.getStringExtra(RedPacketConstant.EXTRA_MONEY_GREETING);
        String moneyID = data.getStringExtra(RedPacketConstant.EXTRA_CHECK_MONEY_ID);
        ...
        message.setAttribute(RedPacketConstant.MESSAGE_ATTR_IS_MONEY_MESSAGE, true);
        message.setAttribute(RedPacketConstant.EXTRA_SPONSOR_NAME, context.getResources().getString(R.string.easemob_red_packet));
        message.setAttribute(RedPacketConstant.EXTRA_MONEY_GREETING, greetings);
        message.setAttribute(RedPacketConstant.EXTRA_CHECK_MONEY_ID, moneyID);
        return message;
    }
    ...
    public static void openRedPacket(final FragmentActivity activity, final int chatType, final EMMessage message, final String toChatUsername, final EaseChatMessageList messageList) {
        ...
        moneyInfo.chatType = chatType;
        LMMoneyManager.getInstance().openMoney(moneyInfo, activity, new LMMoneyManager.LMReceiveMoneyCallBack() {
            @Override
            public void onSuccess(String senderId, String senderNickname) {
                ...
                if (chatType == EaseConstant.CHATTYPE_SINGLE) {
                    ...
                    msg.setAttribute(RedPacketConstant.MESSAGE_ATTR_IS_OPEN_MONEY_MESSAGE, true);
                    msg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_NAME, receiverNickname);
                    msg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_NAME, senderNickname);
                    EMClient.getInstance().chatManager().sendMessage(msg);
                } else {
                    sendMoneyAckMessage(message, senderId, senderNickname, receiverId, receiverNickname);
                }
                messageList.refresh();
            }
    ...
    public static void startChangeActivity(Context context) {
        Intent intent = new Intent(context, LMChangeActivity.class);
        String fromNickname = "";
        String fromAvatarUrl = "";
        EaseUser easeUser = EaseUserUtils.getUserInfo(EMClient.getInstance().getCurrentUser());
        if (easeUser != null) {
     ...
     */
    private static void sendMoneyAckMessage(final EMMessage message, final String senderId, final String senderNickname, String receiverId, final String receiverNickname) {
        //创建透传消息
        final EMMessage cmdMsg = EMMessage.createSendMessage(EMMessage.Type.CMD);
        cmdMsg.setChatType(EMMessage.ChatType.GroupChat);
        EMCmdMessageBody cmdBody = new EMCmdMessageBody(RedPacketConstant.REFRESH_GROUP_MONEY_ACTION);
        cmdMsg.addBody(cmdBody);
        cmdMsg.setReceipt(message.getTo());
        //设置扩展属性
        cmdMsg.setAttribute(RedPacketConstant.MESSAGE_ATTR_IS_OPEN_MONEY_MESSAGE, true);
        cmdMsg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_NAME, senderNickname);
        cmdMsg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_NAME, receiverNickname);
        cmdMsg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_ID, senderId);
        cmdMsg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_ID, receiverId);
        ...
        sendMessage.setAttribute(RedPacketConstant.MESSAGE_ATTR_IS_OPEN_MONEY_MESSAGE, true);
        sendMessage.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_NAME, senderNickname);
        sendMessage.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_NAME, receiverNickname);
        sendMessage.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_ID, senderId);
    ...
    public static void receiveMoneyAckMessage(EMMessage message) {
        ...
        String receiverId = "";
        try {
            senderNickname = message.getStringAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_NAME);
            receiverNickname = message.getStringAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_NAME);
            senderId = message.getStringAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_ID);
            receiverId = message.getStringAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_ID);
        } catch (HyphenateException e) {
            e.printStackTrace();
        }
        ...
        if (currentUser.equals(senderId) && !receiverId.equals(senderId)) {//如果不是自己领取的红包更新此类消息UI
            ...
            msg.setAttribute(RedPacketConstant.MESSAGE_ATTR_IS_OPEN_MONEY_MESSAGE, true);
            msg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_NAME, senderNickname);
            msg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_SENDER_ID, senderId);
            msg.setAttribute(RedPacketConstant.EXTRA_LUCKY_MONEY_RECEIVER_NAME, receiverNickname);
            //保存消息
